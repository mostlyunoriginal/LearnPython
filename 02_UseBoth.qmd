...
Here, we start with `mtcars`, a Polars DataFrame (1). We then apply the `get_column()` method with a reference to the column `cyl` (2), yielding a Polars Series (analogous to the atomic vectors that form the columns of data frames in R). The Series method `unique()` is then applied (3) to yield a new Series with only the distinct values of `cyl`. We then apply the Series method `sort()` to sort our Series by its values (4). This series is assigned to the variable `cyls` (5).
